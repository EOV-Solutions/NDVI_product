...
def fillNA(rvi_data_path, vh_data_path, rvi_full_path, vh_full_path):
    ...
    filled_data[nan_positions] = generic_filter(filled_data, nanmean_filter, size=11)[nan_positions]

    rvi_filled_data = rvi_data.copy()
    for i in range(rvi_data.shape[2]):
        nan_pos = np.isnan(rvi_filled_data[...,i])
        rvi_filled_data[nan_pos,i] = generic_filter(rvi_filled_data[...,i], nanmean_filter, size=11)[nan_pos]

    vh_filled_data = vh_data.copy()
    for i in range(vh_data.shape[2]):
        nan_pos_vh = np.isnan(vh_filled_data[...,i])
        vh_filled_data[nan_pos_vh,i] = generic_filter(vh_filled_data[...,i], nanmean_filter, size=11)[nan_pos_vh]
...
def create_json_data(root_path_data, task_id):
    ...
    def create_ndvi_time_series(folder_path, output_path, missing_dates, date_rvi):
        ...
        while current_date <= end_date:
            ...
            if date_str in missing_dates or not os.path.exists(file_path):
                ...
                null_array = np.full((height, width), -100)
    ...
    def create_sar_time_series(folder_path, output_rvi_path, output_vh_path, date_rvi):
        """
        ...
        for file_name in available_files:
            ...
            with rasterio.open(file_path) as src:
                ...
                vh_time_series.append(src.read(3))  # VH is in band 2
        ...
        logging.info(f"RVI data saved to {output_rvi_path}")
        logging.info(f"VH data saved to {output_vh_path}")
    ...
    # print(missing_date)
    create_ndvi_time_series(folder_path=dir_ndvi, output_path=ndvi_time_series_path, missing_dates=missing_date, date_rvi=date_rvi)
    create_sar_time_series(folder_path=dir_rvi, output_rvi_path=rvi_time_series_path, output_vh_path=vh_time_series_path, date_rvi=date_rvi)
    ...
    for series_index in range(ndvi_data.shape[0]):
        for time_step in range(ndvi_data.shape[1]):
            if ndvi_data[series_index, time_step] == -100:
    ...
    def cleanup_data_folder(data_folder):
        ...
        Parameters:
            data_folder (str): Path to the folder to clean up.
        """
        for root, dirs, files in os.walk(data_folder, topdown=False):
            ...
            for dir in dirs:
                ...
                # Kiểm tra nếu thư mục rỗng sau khi xóa file bên trong thì mới xóa
                if not os.listdir(dir_path):
                    shutil.rmtree(dir_path)
        logging.info(f"Cleanup completed for folder: {data_folder}")


    # cleanup_data_folder("/app/data")
